...
## Custom Review Standards Check (sync-baked)

**The project's team review rules are inlined into the reviewer's prompt at
`specpower init`/`sync` time** — the `Custom Standards` placeholder in the
Code Reviewer Template below is replaced with the concatenated contents of
`specpower/custom/review/` top-level `.md` (lexicographic; subdirs/non-.md
ignored); missing/empty → `none`. The reviewer does NOT read
`specpower/custom/review/` itself (subagents receive full text, not file-fetch
tasks). The reviewer applies these as ADDITIONAL review dimensions on top of
the built-in checklist, flagging violations at each rule's specified severity
(`[Critical]` / `[Important]` / `[Minor]`, default Important). For
project-convention conflicts (naming, structure, style) the custom rule wins;
built-in safety/correctness checks always apply regardless of custom standards.

If the placeholder is still literally present at dispatch time (sync bake
missing/failed), the reviewer reports DONE_WITH_CONCERNS (D9 self-check).
...
Task tool (general-purpose):
  ...
  prompt: |
    ...
    **Custom Standards (sync-baked — do NOT read specpower/custom/review/ yourself):**
    ...
    (lexicographic; subdirs/non-.md ignored). If missing or empty, it reads `none`.
    [CONTROLLER: paste review rules here]
    - Apply each rule as an additional review dimension on top of the checklist above
    - Flag violations at the severity the rule specifies (default Important)
    - Custom conventions override built-in style guidance; built-in safety/correctness always applies
    - **D9 self-check:** if you see literal `[CONTROLLER:` text (sync bake missing/failed), report DONE_WITH_CONCERNS — do not treat it as a rule
